...
WorldGenerator::WorldGenerator(BlocksManager &blockManager,
  ...
  node = FastNoise::NewFromEncodedNodeTree(
...
std::shared_ptr<Chunk> WorldGenerator::generateChunk(int cx, int cz) {
  ...
  heightGenFBm->GenUniformGrid2D(heights.data(), cx * Chunk::CHUNK_SIZE,
                                 cz * Chunk::CHUNK_SIZE, Chunk::CHUNK_SIZE,
                                 Chunk::CHUNK_SIZE, 0.005f, 1337);
  tempNoise->GenUniformGrid2D(temps.data(), cx * Chunk::CHUNK_SIZE,
                              cz * Chunk::CHUNK_SIZE, Chunk::CHUNK_SIZE,
  ...
  for (int z = 0; z < Chunk::CHUNK_SIZE; ++z) {
    for (int x = 0; x < Chunk::CHUNK_SIZE; ++x) {
      ...
      int height = static_cast<int>((heights[activeIdx] + 1.0f) *
      ...
      for (int y = 0; y < std::max(height, waterLevel); ++y) {
        ...
        chunk->setBlock(x, y, z,
                        temps[activeIdx] > 0.5 ? BlockId::CoalOre
                                               : BlockId::Grass);
        if (y == 0) {
