...
SwapChainVk::SwapChainVk(RenderDeviceVk *device, vk::Extent2D extent) : m_device{device}, m_windowExtent{extent} {
  ZoneScoped;
  init();
}

SwapChainVk::SwapChainVk(RenderDeviceVk *device, vk::Extent2D extent, std::shared_ptr<SwapChainVk> previousSwapChain)
...
vk::Result SwapChainVk::submitCommandBuffers(const vk::CommandBuffer *buffers, uint32_t *imageIndex) {
  ZoneScoped;
...
vk::PresentModeKHR SwapChainVk::chooseSwapPresentMode(const std::vector<vk::PresentModeKHR> &availablePresentModes) {
  ZoneScoped;
...
vk::Extent2D SwapChainVk::chooseSwapExtent(const vk::SurfaceCapabilitiesKHR &capabilities) {
  ZoneScoped;
